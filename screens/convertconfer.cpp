...
{
  readJSON(schedule,"schedule.json","schedule");
  readJSON(sessions,"sessions.json","sessions");
  readJSON(papers,"papers.json","entities");
  readJSON(extra,"extra.json","extra");
}
...
static void dumpSession(ostream& out2,string room,string sessionName,string sessionLabel, string day, string time)
{

  string filename = "sources/session-" + room + "-" + day + "-" + time + ".tex";
  ...
  out << "\\input{template.tex}\n";  

  out << "\\begin{minipage}{\\textwidth}\n";
  out << "\\slotheading{" << day << " " << time << " -- " << room << "}" << endl << endl;
...
static set<string> getIgnoreDays()
{
  ...
  {
    ignoreDays.insert("06/30/2019");
    ignoreDays.insert("07/05/2019");
  }
  return ignoreDays;
}
